...
void spec_advance( t_species* spec, t_emf* emf, t_current* current )
{
    ...
    #pragma omp parallel reduction(+:energy)
    {
        ...
        current_new( &thread_current, current->nx, current->box, spec->dt );
        
         #pragma omp for nowait schedule(static, 10000)
        for (int i=0; i<spec->np; i++) {
        ...
        #pragma omp critical
        {
            // Add thread currents to global currents
            int total_size = current->gc[0] + current->nx + current->gc[1];
            for (int i = 0; i < total_size; i++) {
                current->J_buf[i].x += thread_current.J_buf[i].x;
                current->J_buf[i].y += thread_current.J_buf[i].y;
                current->J_buf[i].z += thread_current.J_buf[i].z;
            }
        }
        
        // Free the thread-local current buffer
        free(thread_current.J_buf);
    }  // <<< This closes the parallel region
