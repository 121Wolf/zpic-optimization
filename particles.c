...
//we declare J threads as Global to reutilize and free them at the end of the programm
static float3** J_threads = NULL;
static int allocated_threads = 0;
static int allocated_J_size = 0;
 ...
 * @param current   Electric current density
 */
void dep_current_zamb_array( int ix0, int di,
                        float x0, float dx,
                        float qnx, float qvy, float qvz,
                        float3* J )
{
              ...
              const t_part* restrict const part, float3* restrict const Ep, float3* restrict const Bp )
{
    ...
    // Branchless indexing
    int ih = i - (w1 >= 0.5f);  // Equivalent to original but branchless
    float w1h = w1 + (0.5f - (float)(w1 >= 0.5f));  // w1 + 0.5f or w1 - 0.5f
    
    // Load ALL needed E values at once
    float3 E_i = E[i];
    float3 E_i1 = E[i+1];
    float3 E_ih = E[ih]; 
    float3 E_ih1 = E[ih+1];
    
    // Load ALL needed B values at once  
    float3 B_i = B[i];
    float3 B_i1 = B[i+1];
    float3 B_ih = B[ih];
    float3 B_ih1 = B[ih+1];
    
    // Now compute with registers only
    float wx[2] = {1.0f - w1, w1};
    float wxh[2] = {1.0f - w1h, w1h};
    
    Ep->x = E_ih.x * wxh[0] + E_ih1.x * wxh[1];
    Ep->y = E_i.y * wx[0] + E_i1.y * wx[1];
    Ep->z = E_i.z * wx[0] + E_i1.z * wx[1];
    
    Bp->x = B_i.x * wx[0] + B_i1.x * wx[1];
    Bp->y = B_ih.y * wxh[0] + B_ih1.y * wxh[1];
    Bp->z = B_ih.z * wxh[0] + B_ih1.z * wxh[1];
}
...
{  
    ...
    #pragma omp parallel reduction(+:energy)
    {
        ...
        for (int i = 0; i < spec->np; i++) {
            ...
            dep_current_zamb_array( spec -> part[i].ix, di,
                                  spec -> part[i].x, dx,
                                  qnx, qvy, qvz,
                                   J_local );
    ...
    for (int t = 0; t < num_threads; t++) {
       for (int i = 0; i < J_size; i++) {
        ...
        current->J[i].y += J_threads[t][i].y;
        current->J[i].z += J_threads[t][i].z;
    }
} 
...
/**
 * @bried Allocates memory for threads and corresponding float3 array
 *
 *
 */
void init_current_buffers(int max_threads, int max_J_size) {
    if (J_threads) {
    ...
    J_threads = malloc(max_threads * sizeof(float3*));
    for (int t = 0; t < max_threads; t++) {
        J_threads[t] = malloc(max_J_size * sizeof(float3));
    }
    allocated_threads = max_threads;
    allocated_J_size = max_J_size;
}


/**
 * @bried Frees up memory from all arrays and threads used in the simulation
 *
 */
