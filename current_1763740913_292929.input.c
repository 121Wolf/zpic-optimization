...
___POMP2_INCLUDE___
#line 1 "/projects/F202500010HPCVLABUMINHO/jose.monteiro/trab/zpic_CompParalela-main/current.c"
/**
 * @file current.c
...
//initializes with everything zeroed
void current_init( t_current *current, int nx, float box, float dt ) {
    ...
    current->J_buf = calloc( size, sizeof( float3 ) );  // Note: calloc to zero memory
    assert( current->J_buf );
