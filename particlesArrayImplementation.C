...
{  
    ...
    if(!J_threads || allocated_threads != num_threads || allocated_J_size != J_size) {
        // Free old arrays
        if (J_threads) {
            for (int t = 0; t < allocated_threads; t++) free(J_threads[t]);
            free(J_threads);
        }
        
	 // Allocate new arrays
        J_threads = malloc(num_threads * sizeof(float3*));
        for (int t = 0; t < num_threads; t++) {
            J_threads[t] = malloc(J_size * sizeof(float3));
        }
        allocated_threads = num_threads;
        allocated_J_size = J_size;
    }
    ...
    #pragma omp parallel reduction(+:energy)
    {
        ...
        #pragma omp for schedule(static, 512) nowait
        for (int i=0; i<spec->np; i++) {
